...
def args_init(printArgs: bool = False) -> bool or list:
    try:
        ...
        parser.add_argument('commandnumber', type=int, nargs='*',
                            help='Command numbers to quickly exceute.')
        args = parser.parse_args()
        if printArgs:
            print(args.commandnumber)
...
def confirm_execution(command_name, command) -> bool:
    while True:
        print()
        print('', wrap_string("Command name:", front=Fore.YELLOW),
...
def executer(index):
    ...
    if user_choice - 1 in range(len(data)):
        ...
        print(wrap_string("Enter a valid number from 1 to {}.".format(
            len(data)), front=Fore.RED))
